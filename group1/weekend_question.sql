...
SELECT tran_date, tran_ammt, SUM(tran_ammt)
OVER (PARTITION BY tran_date) as total_transaction_ammount,
DENSE_RANK() over (PARTITION BY tran_date
...
FROM ( 
	SELECT *, 
	SUM(tran_ammt) OVER (PARTITION BY tran_date) as total_transaction_ammount,
	DENSE_RANK() over (PARTITION BY tran_date ORDER BY tran_ammt desc) as ranknum
	FROM cards_ingest.tran_fact fact INNER JOIN cards_ingest.cust_dim_details dim 
	ON fact.cust_id = dim.cust_id AND fact.tran_date BETWEEN dim.start_date AND dim.end_date
	) inq
WHERE ranknum = 2
ORDER BY tran_date
...
SELECT *,
	CASE
	WHEN COALESCE(stat_cd,'aa') != COALESCE(state_cd, 'bb') THEN 'data issues'
	ELSE 'good data'
...
FROM 
	...
	OVER (PARTITION BY tran_date) as total_transaction_ammount,
	DENSE_RANK() over (PARTITION BY tran_date
	ORDER BY tran_ammt desc) ranknum
	FROM cards_ingest.tran_fact fact
	INNER JOIN cards_ingest.cust_dim_details dim 
	ON fact.cust_id = dim.cust_id AND fact.tran_date BETWEEN dim.start_date AND dim.end_date
	) inq
